...
NanoRoute.AwsLambda adds AWS Lambda adapters for NanoRoute while keeping the core package transport-neutral. For custom transports, use the core package's `RequestPipeline` to run the shared `HttpRequestMessage` pipeline from your own router type.
